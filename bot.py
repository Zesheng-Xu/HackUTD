...
async def on_message(message):
    ...
    if message.content.startswith('.event '):
        ...
        if (message.channel.name.strip(" ") == "calendar"):

            category_name = message.channel.category.name.replace(" ","").upper
            ...
            if len(contents) != 5:
                ...
                cevent = myCalendar()
                cevent.devent(contents[1].strip(" ").replace(" ","").upper(), contents[2].strip(" ").replace(" ","").upper(), contents[3].strip(" ").replace(" ","").upper(), contents[4].strip(" ").replace(" ","").upper())
                for e in events:
                        if e.equals(cevent):
                                already_e = True
            ...
            if message.content.startswith('.due '):
                    ...
                    if(len(contents) == 1):
                        ...
                        for e in events:
                            if contents[1] == e.get_class() and contents[2] == e.get_professor() and contents[3] == e.get_event():
                                t = str(time.localtime(time.time()))
                                temp = t.split(",")
                                month = temp[1][temp[1].rfind("=") + 1:]
                                day = temp[2][temp[2].rfind("=") + 1:]
                        ...
                        for e in events:
                            if contents[1] == e.get_class() and contents[2] == e.get_professor() and contents[3] == e.get_event() and contents[4] == e.get_date():
                                t = str(time.localtime(time.time()))
                                temp = t.split(",")
                                month = temp[1][temp[1].rfind("=") + 1:]
                                day = temp[2][temp[2].rfind("=") + 1:]
    ...
    if message.channel.name == "bot-commands":
        ...
        if message.content.startswith('.init'): #and message.author.id == message.guild.owner_id:
            ...
            for key in data:
                ...
                await generateRole(message.guild, category, key)
